# Supermikrokontroler v2 — Pełna Dokumentacja
...
```
  ...
  │                                 │                            │  ├── cmd_i2c.h             │
  │  Peripherals available:         │                            │  ├── cmd_spi.h             │
...
| STM32F103C8T6 Blue Pill | Klon OK; weryfikuj flash 64 KB (nie 128 KB) |
| 4× przewód Dupont F-F | Maks. 20 cm dla stabilności |
| stm32flash | `get_stm32flash.ps1` / `get_stm32flash.sh` |
...
⚠ UWAGA: Nigdy nie podłączaj 5V do żadnego pinu STM32.
...
| `GPIO:TOGGLE:PIN` | Toggle output | nowy stan `0`/`1` |
...
⚠ Flash STM32F103 ma ~10 000 cykli kasowania. Nie pisz w pętli szybszej niż co 100 ms.
...
| Komenda DATA | Opis | Wynik |
|-------------|------|-------|
| `SYS:STATUS` | Wersja, uptime, RAM, reset cause | `v2.0:UP:12345:RAM:8192:RST:POR:CPU:72MHz:ID:AABBCCDD` |
| `SYS:UPTIME` | ms od boot | `12345` |
| `SYS:CHIPID` | 96-bit unique device ID | `A1B2C3D4E5F60102...` (24 hex) |
...
Ustawienia Serial Monitor: **115200 baud**, zakończenie linii **Newline**.
...
can nowarn              wyłącz ostrzeżenie o transceiverze
can warn                przywróć ostrzeżenie
can tx  <id> <hexbytes>
can txe <id> <hexbytes>
can rx
...
# NTP
ntp sync                        pobierz czas NTP → wyślij do slave RTC
ntp status
ntp server <hostname>

# System
sys status | sys uptime | sys chipid | sys cpufreq
sys fwver  | sys freeram
sys echo <text>
sys reset
...
Wszystkie metody są **blokujące** — czekają na odpowiedź slave (max `STM32_API_TIMEOUT_MS` = 6 s).
...
```cpp
stm.gpio.mode("A0", GPIO_ANALOG);     // GPIO_IN, GPIO_OUT, GPIO_IN_PU,
                                       // GPIO_IN_PD, GPIO_ANALOG, GPIO_OD
stm.gpio.write("C13", 0);             // LOW — LED on (active-low)
int v  = stm.gpio.read("A1");         // 0 lub 1
int v2 = stm.gpio.toggle("C13");      // flip, zwraca nowy stan
int byte = stm.gpio.port('A');        // dolne 8 bitów portu A jako int
```

### ADC

```cpp
int raw  = stm.adc.read("A0");        // 0–4095
int avg  = stm.adc.avg("A0", 16);     // średnia 16 próbek
int mv   = stm.adc.mv("A0");          // mV

int vals[4];
int n = stm.adc.multi("A0,A1,B0,B1", vals, 4);

String hex = stm.adc.stream("A0", 32, 1); // 32 próbki co 1 ms, packed hex
// Dekodowanie: co 4 znaki = jedna próbka uint16
for (int i = 0; i < 32; i++) {
    uint16_t s = strtoul(hex.substring(i*4, i*4+4).c_str(), nullptr, 16);
}
...
```cpp
stm.pwm.set("A8", 500);           // 50% duty, domyślna częstotliwość
stm.pwm.freq("A8", 1000, 750);    // 1 kHz, 75% duty
stm.pwm.stop("A8");
int duty = stm.pwm.read("A8");    // ostatnio ustawiony duty
```

### I2C

```cpp
// I2C1 (PB6/PB7), I2C2 (PB10/PB11) — te same metody
stm.i2c.setSpeed(400);                    // 100 lub 400 kHz
String addrs = stm.i2c.scan();            // "0x3C,0x68" lub "NONE"
bool found   = stm.i2c.ping(0x68);

stm.i2c.writeReg(0x68, 0x6B, "00");      // MPU6050 wake up
String data  = stm.i2c.readReg(0x68, 0x3B, 6); // 6 bytes → hex
...
```cpp
stm.spi.begin("B12", 0, 5000);        // CS=PB12, mode 0, 5 MHz
String rx = stm.spi.xfer("B12", "FF00"); // full-duplex
stm.spi.end();
```
...
```cpp
stm.ee.write(0, 0xAB);
int val = stm.ee.read(0);              // 171

stm.ee.writeWord(4, 0xDEADBEEF);
uint32_t w = stm.ee.readWord(4);

stm.ee.writeHex(16, "CAFEBABE");
String hex = stm.ee.readHex(16, 4);   // "CAFEBABE"
```
...
if (events != "NONE") {
    ...
    int count = events.substring(colon + 1).toInt();
    Serial.printf("A0 triggered %d times\n", count);
}
...
Serial.println(stm.rtc.get());         // "2025-06-15 14:30:05 Sun"
uint32_t ts = stm.rtc.getTimestamp();  // sekundy od 2000-01-01
uint32_t ep = stm.rtc.epoch();         // Unix timestamp
stm.rtc.setTimestamp(ts + 3600);       // przesuń o godzinę
```

### CAN

```cpp
stm.can.setNoWarn(true);               // wyłącz ostrzeżenie o transceiverze
stm.can.begin(250, "LOOPBACK");        // bez transceivera

stm.can.tx(0x123, "DEADBEEF");
String frame = stm.can.rx();           // "291:DEADBEEF:0:0" lub "NONE"

stm.can.setFilter(0x100, 0x7F0);       // akceptuj 0x100–0x10F
stm.can.clearFilter();
stm.can.end();
```
...
Serial.println(stm.sys.chipId());
long uptimeMs = stm.sys.uptime();
int  freeMem  = stm.sys.freeRam();
...
if (!stm.ok()) {
    ...
    // error() zwraca payload z ramki ERR
}

// Raw wynik (zawsze):
String raw = stm.lastRaw();

// Escape hatch — dowolna komenda nie ma wrappera:
...
```cpp
// Zły wzorzec — blokuje loop() na długo:
for (int i = 0; i < 100; i++) {
    stm.ee.write(i, 0xFF);   // 100 × ~10 ms = 1 s blokady
}

// Dobry wzorzec — jedna operacja na iterację:
static int eeIdx = 0;
if (eeIdx < 100) {
    stm.ee.write(eeIdx++, 0xFF);
}
// stm.pump() wywoływane przez write() wewnętrznie
```
...
3. Oblicza epoch-2000 = unix_time − 946684800
...
```cpp
// W setup() po wifi.connect():
if (stm.rtc.init()) {
    // ntpSync() z wifi_ntp.h uruchamia automatyczny łańcuch RTC:SETTSS
    ntpSync();  // nieblokujące — ntpTick() w loop() dokończy sekwencję
}
```
...
```
can begin 250 loopback    ← TX→RX wewnętrznie; PB8/PB9 nic nie muszą mieć
can tx 123 DEADBEEF
...
```
can nowarn                ← wyłącz ostrzeżenie (jeśli transceiver jest podłączony)
...
| Tryb | Transceiver | TX aktywne | RX aktywne | Zastosowanie |
|------|-------------|-----------|-----------|-------------|
...
| PB8 | CAN1 RX (remapped) | TIM4 CH3 | CAN ↔ PWM |
| PB9 | CAN1 TX (remapped) | TIM4 CH4 | CAN ↔ PWM |
...
# Dłuższy timeout:
.\test_harness.ps1 -Port COM3 -Timeout 10
```
...
python test_harness.py              # auto-detekcja
python test_harness.py /dev/ttyUSB0
python test_harness.py COM3
```
...
Normalny objaw — linia DTR triggeruje ESP32 EN. Wizard PS7 (`flash_script.ps1`)
...
Nie każdy pin Blue Pill obsługuje PWM sprzętowo. Użyj pinów z sekcji "PWM-capable"
w rozdziale 5. Na nieprawidłowym pinie nie pojawi się błąd — `analogWrite()` ustawi
stan cyfrowy.

### `ee write` wolne lub reset STM32
Każdy zapis commituje stronę flash (~10 ms). Nie pisz szybciej niż co 100 ms.
Po przekroczeniu ~10 000 cykli na stronę EEPROM przestanie działać poprawnie.

### CAN: `CAN:INIT_FAIL`
Sprawdź: 1) AFIO clock aktywny (w kodzie tak jest), 2) PB8/PB9 nie używane przez
inne peryferium (BOOT0 na PB2, TIM4 na PB8/PB9 — muszą być wolne).

### RTC: `RTC:LSE_FAIL` lub ciągłe `NOT_SET`
Kryształ 32.768 kHz musi być fizycznie wlutowany na PC14/PC15. Wizualnie
to mały srebrny element (szary kryształ ceramiczny) obok CPU.
Jeśli go brakuje, RTCnie startuje — użyj `RTC:INIT` po podłączeniu kryształu.

### API: `stm.begin()` zwraca false
Slave nie odpowiada w ciągu `timeoutMs`. Sprawdź to samo co przy braku PONG.
Upewnij się, że `stm.pump()` jest w `loop()` i nie blokuje.
...
│   ├── stm32_api.h             ★ Programistyczne C++ API (STM32 class)
│   └── example_api_usage.ino  Przykłady użycia API (10 scenariuszy)
│
├── stm32_slave/
...
│
├── get_stm32flash.ps1          Pobieranie stm32flash z weryfikacją MD5 (PS7)
├── get_stm32flash.sh           Pobieranie stm32flash z weryfikacją MD5 (bash)
├── flash_script.ps1            Wizard flashowania STM32 przez ESP32 bridge (PS7)
├── flash_stm32.bat             Prosty wrapper flashowania (Windows)
├── flash_stm32.sh              Prosty wrapper flashowania (Linux/macOS)
├── test_harness.ps1            Smoke test (PS7) — 40+ asercji, auto COM detect
├── test_harness.py             Smoke test (Python) — ta sama matryca
├── README.md                   Skrócony przewodnik startowy
